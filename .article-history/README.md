...
### メンテナンス責任者
**Claude**（crow-bot のスケジューラジョブ `ai-news-digest-generate`）が
このファイルを自動的にメンテナンスします。

- 毎日のニュース収集タスク実行時に、Claude が以下の操作を実行
- 人間による手動編集は基本的に不要です
- 生成手順は `CLAUDE.md` に記載されています
